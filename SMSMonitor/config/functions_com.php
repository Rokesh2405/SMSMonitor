...
function addtax($title, $percentage, $type, $order, $status, $ip, $getid) {
    ...
    if ($getid == '') {
        ...
        if ($link22['title'] == '') {
            ...
            $resa->execute(array($title, $percentage, $type, $order, $status, $ip, $_SESSION['VEGID']));
            
            $id=$db->lastInsertId();
            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");
            $htry->execute(array('Tax Mgmt', 42, 'INSERT', $_SESSION['VEGID'], $ip, $id));
        ...
        if ($link22['title'] == '') {
            ...
            $resa->execute(array(trim($title),trim($type), trim($percentage), trim($order), trim($status), trim($ip), $_SESSION['VEGID'], $getid));
			
            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");
            $htry->execute(array('Tax Mgmt', 42, 'UPDATE', $_SESSION['VEGID'], $ip, $id));
